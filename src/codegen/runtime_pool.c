...
" * 8-worker pool burned 0.83s of CPU per 10s of wall clock (~8% of a",
" * core, permanently, doing nothing) against 0.04s for the single-",
" * global-queue design it replaced -- 8 workers x 1000 wakeups/sec x",
" * ~6 mutex acquisitions each. Real programs in this repo idle exactly",
" * this way between requests (demo/main.sl, examples/httpd), so that",
" * is a genuine regression, not a synthetic one.",
" *",
...
" * can then be long (50ms) instead of 1ms. Measured back down to 0.05s",
" * per 10s idle -- baseline -- with no added wakeup latency.",
" *",
...
" * this call stubbed out showed it dominating the difference, but that",
" * run was taken on a machine under heavy unrelated load and no figure",
" * from it is quoted here; the gate is justified by the argument below",
" * regardless, and the clean re-measurement is still owed (todo.md).",
" *",
...
" * ~8% of a core doing nothing (see sl_pool_idle_mu's own comment for",
...
" * is what should make per-worker queues pay for themselves. The",
" * benchmark run that drove this change was taken under heavy",
" * unrelated machine load, so it is deliberately not quoted as a",
" * number -- the A/B on a quiet machine is still owed (todo.md).",
" *",
...
"     * a window the quiescence protocol already closes. The measurement",
"     * that motivated dropping it was taken on a machine that turned",
"     * out to be under heavy unrelated load, so no number is quoted",
"     * here -- the justification is the argument above, not that",
"     * benchmark, and the A/B is still owed on a quiet machine (see",
"     * todo.md). Note this is exactly why sl_task_resume",
...
"                waking. Long on purpose: at 1ms this cost ~8% of a core",
"                per idle pool, measured. */",
