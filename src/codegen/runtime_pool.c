...
"    /* Unconditional -- sl_pool_wake_one's own seq_cst load of",
"     * sl_pool_idle_count already makes this free when nobody is idle,",
"     * which is the whole steady state of a busy pool, and it takes the",
"     * mutex ONLY when there is genuinely an idle worker to wake, i.e.",
"     * exactly when waking matters.",
"     *",
"     * This was previously gated on was_empty (this push taking the",
"     * queue from empty to non-empty), on the argument that a sleeping",
"     * worker implies every queue is empty, so the next push must be a",
"     * 0->1 transition and will signal. That argument is sound for the",
"     * FIRST push of a burst and wrong for the rest: N workers can be",
"     * asleep, and pushes 2..M land on a now-non-empty queue, signal",
"     * nobody, and leave the other N-1 workers asleep until the 50ms",
"     * backstop fires. Under a server's concurrent I/O completions that",
"     * is a direct tail-latency source, and it is what remained of the",
"     * HTTP p99 regression after the global-first fix below.",
"     *",
"     * The gate was introduced to avoid mutex traffic per push, on a",
"     * measurement taken while this machine was under heavy unrelated",
"     * load -- see this file's history. The idle_count check already",
"     * buys that, so the gate was paying latency for nothing. */",
"    sl_pool_wake_one();",
"}",
"",
...
"    /* GLOBAL FIRST, then this worker's own queue, then steal. The",
"     * order matters far more than it looks, and the obvious",
"     * locality-first order (own queue first) is what the HTTP",
"     * acceptance test caught as a 4-7x tail-latency regression.",
"     *",
"     * The two queues carry different KINDS of work, which is the part",
"     * that makes local-first pathological rather than merely",
"     * suboptimal. sl_task_resume pushes to the GLOBAL queue -- that is",
"     * every task woken by an I/O completion, a chan handoff, or a",
"     * timer, i.e. in a server essentially every request with a client",
"     * blocked on the other end of it. sl_worker_after_switch's",
"     * preempted branch and sl_task_submit push to the LOCAL queue.",
"     * Draining local first therefore lets preempted and freshly-spawned",
"     * work indefinitely outrank work someone is actually waiting on:",
"     * throughput stays flat (the same tasks run either way) while p99",
"     * collapses, which is exactly the measured signature -- 859-927ms",
"     * baseline against 3333-6202ms with local-first, at equal rps.",
"     *",
"     * Checking global first costs one extra uncontended try-pop on the",
"     * shared queue per dispatch. That is the cost work-stealing exists",
"     * to avoid, so it is worth being explicit that it is being paid",
"     * back deliberately: per-worker queues still buy locality for the",
"     * steal path and for fan-out, and the global queue is no longer the",
"     * only place work can live -- but it is where the latency-critical",
"     * work lives, so it must be looked at first. */",
"    sl_task *t = sl_runq_try_pop(&sl_global_runq);",
"    if (t) return t;",
"    t = sl_runq_try_pop(&sl_worker_runq[slot_idx]);",
"    if (t) return t;",
