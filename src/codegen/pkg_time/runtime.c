...
const char *TIME_RUNTIME[] = {
    ...
    " * imported at all. block_signals mirrors sl_pool_start's own",
    " * parameter exactly and for the identical reason: this is a new,",
    " * always-running background OS thread, and without blocking",
    " * SIGTERM/SIGINT here too it would become a second thread capable",
    " * of receiving a signal meant only for main() -- reopening exactly",
    " * the hazard sl_pool_start's own block_signals already guards",
    " * against for pool workers (tests/proc_shutdown's own load-bearing",
    " * 'only main can get it' requirement). */",
    "static void sl_time_start(int block_signals) {",
    ...
    "    pthread_mutex_lock(&sl_time_mu);",
    "    sl_time_insert(sl_rt_current_task, sl_time_mono() + ns);",
