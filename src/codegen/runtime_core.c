...
const char *RUNTIME[] = {
    ...
    " * uses). Nothing walks this chain yet -- GC_malloc/Boehm stays",
    " * authoritative until the allocator swap -- so this is purely",
    " * additive bookkeeping the precise collector will consume later.",
    " * _Thread_local like sl_rt_is_main_thread above, so every OS",
    " * thread (main, the worker pool, any spawn trampoline) gets its",
    " * own independent, correctly zero-initialized chain for free. */",
