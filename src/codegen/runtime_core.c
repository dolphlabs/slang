...
const char *RUNTIME[] = {
    ...
    "    /* Growth trigger. MUST come after the four lines above, not",
    "     * before: sp/roots are raw addresses into the caller's own (soon-",
    "     * to-be-relocated-if-we-grow) stack frame, received as plain",
    "     * pointer PARAMETERS -- once linked into the chain via the",
    "     * assignments above, sl_task_stack_grow's own chain walk reads",
    "     * and fixes them up correctly (same mechanism as the collector's",
    "     * root scan); the sp/roots C parameters themselves must never be",
    "     * dereferenced again after a grow, and nothing below does --",
    "     * only sl_rt_current_task->safepoint_top (itself correctly",
    "     * translated) is read from here on. wrap_safepoint (core.c) never",
    "     * writes back through &_sl_spN/_sl_spN_roots after this call",
    "     * either, confirmed directly before wiring this in. */",
    "    {",
    "        char sl_rt_stack_probe;",
    "        if ((uintptr_t)&sl_rt_stack_probe -",
    "                (uintptr_t)sl_rt_current_task->stack_base <",
    "            SL_TASK_GUARD_MARGIN)",
    "            sl_task_stack_grow(sl_rt_current_task);",
    "    }",
    "    sl_rt_gc_checkin();",
    "}",
    "",
