...
const char *RUNTIME[] = {
    ...
    "    sl_task *head, *tail;",
    "    /* Tier 11 stretch (work-stealing): number of tasks currently on",
    "     * THIS queue, maintained under mu by every push and pop. Distinct",
    "     * from sl_global_runq_count, which is a single relaxed, heuristic",
    "     * pool-wide total and cannot answer 'how deep is this particular",
    "     * queue'. Needed so a stealer can take half a victim's backlog in",
    "     * one lock acquisition instead of one task per acquisition -- see",
    "     * sl_runq_steal_half (runtime_pool.c) for why that is a",
    "     * correctness-of-performance requirement rather than a tuning",
    "     * nicety. Plain long, not atomic: only ever read or written with",
    "     * mu held. */",
    "    long count;",
    "    pthread_mutex_t mu;",
    ...
    "",
    "/* Tier 11 stretch: work-stealing per-worker run queues. Declared here",
    " * (not in runtime_pool.c, where sl_pool_start/sl_worker_run_loop and",
    " * the rest of the pool machinery live) for the identical ordering",
    " * reason as sl_global_runq itself just above -- sl_gc_collect's own",
    " * run-queue walk (runtime_gc.c) needs to root every worker's own",
    " * queue too, not just the global one, and that code is emitted",
    " * before RUNTIME_POOL. SL_POOL_MAX_WORKERS moves here from its old",
    " * spot in runtime_pool.c for the same reason -- sl_worker_runq[]'s",
    " * own size needs it at this definition site.",
    " *",
    " * sl_worker_runq[i] is worker slot i's own local run queue -- same",
    " * sl_runq type as sl_global_runq, reused unchanged rather than a new",
    " * lock-free structure, deliberately: this codebase's own history",
    " * (the async-preemption slice above found eleven real concurrency/",
    " * memory-safety bugs in adjacent scheduler code) argues strongly for",
    " * the more conservative choice here. sl_global_runq itself is NOT",
    " * replaced -- it stays as the overflow/fallback queue for pushers",
    " * with no worker slot of their own (sl_task_resume, called from the",
    " * timer/reactor threads; any pusher running with slot_idx == -1,",
    " * i.e. main's own original OS thread).",
    " *",
    " * sl_worker_runq[]'s elements are NOT validly usable via their",
    " * static zero-initialization alone -- see sl_pool_start's own",
    " * comment (runtime_pool.c) for why an explicit pthread_mutex_init/",
    " * pthread_cond_init pass is required before any worker touches",
    " * them, a real, Darwin-specific hazard (PTHREAD_MUTEX_INITIALIZER/",
    " * PTHREAD_COND_INITIALIZER are not all-zero-bytes on this platform,",
    " * unlike Linux/glibc where zero happens to work by coincidence).",
    " *",
    " * sl_pool_num_workers is set exactly once, by sl_pool_start, BEFORE",
    " * its own pthread_create loop begins (POSIX guarantees pthread_create",
    " * synchronizes-with the new thread's start, so a plain, non-atomic",
    " * long written before that call is safely visible to every worker",
    " * with no fence needed -- the same reasoning sl_pool_workers[]'s own",
    " * sizing already relies on). Every access to sl_worker_runq[] --",
    " * both the GC's own walk and the pop/steal logic -- MUST be bounded",
    " * by sl_pool_num_workers, never by SL_POOL_MAX_WORKERS: indices at",
    " * or past sl_pool_num_workers have UNINITIALIZED mutexes/condvars,",
    " * exactly the same hazard as touching them before sl_pool_start's",
    " * own init pass runs at all. This is an easy copy-paste mistake --",
    " * sl_pool_slots[] (runtime_pool.c), which this array's own per-slot",
    " * shape otherwise mirrors, IS safe to scan across the full fixed 256",
    " * slots, since its own fields are plain atomics, safely NULL/0 when",
    " * unused -- sl_worker_runq[] has no such luxury once its mutexes are",
    " * explicitly initialized rather than left at their zero-fill. */",
    "#define SL_POOL_MAX_WORKERS 256",
    "static sl_runq sl_worker_runq[SL_POOL_MAX_WORKERS];",
    "static long sl_pool_num_workers = 0;",
    "",
