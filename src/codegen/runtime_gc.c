...
const char *RUNTIME_GC[] = {
    ...
    "}",
    "",
    "/* Tier 11 stretch (work-stealing): the run-queue-walk body sl_gc_collect",
    " * used to have inline, extracted into its own function so it can be",
    " * called once per queue -- the global overflow queue, plus every",
    " * worker's own local queue (runtime_core.c's sl_worker_runq[]),",
    " * rather than assuming a single sl_global_runq holds every queued",
    " * task. Locks q's own mutex for the duration of its own walk, one",
    " * queue at a time -- sl_gc_collect (below) calls this sequentially,",
    " * never holding two different queues' mutexes at once, so this adds",
    " * no new deadlock risk beyond what a single sl_global_runq walk",
    " * already had. */",
    "static void sl_gc_scan_runq(sl_runq *q) {",
    "    pthread_mutex_lock(&q->mu);",
    "    for (sl_task *sl_gc_qt = q->head; sl_gc_qt;",
    ...
    "    sl_gc_scan_runq(&sl_global_runq);",
    "    for (long sl_gc_wi = 0; sl_gc_wi < sl_pool_num_workers; sl_gc_wi++)",
    "        sl_gc_scan_runq(&sl_worker_runq[sl_gc_wi]);",
